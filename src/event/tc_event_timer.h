...
tc_event_timer_t* tc_event_add_timer(tc_pool_t *pool, 
        tc_msec_t timer, void *data, tc_event_timer_handler_pt handler);
...
tc_event_update_timer(tc_event_timer_t *ev, tc_msec_t timer)
{
    ...
    if (ev != NULL) {
        ...
        tc_log_debug1(LOG_DEBUG, 0, "update timer:%llu", ev);
