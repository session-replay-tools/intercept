...
tc_intercept_release_tunnel(int fd, tc_event_t *rev)
{
    if (srv_settings.tunnel[fd].fd_valid) {
        tc_log_info(LOG_NOTICE, 0, "release tunnel resources, fd:%d", fd);
        tc_socket_close(fd);           
        srv_settings.tunnel[fd].fd_valid = 0;
#if (TC_COMBINED)
        if (srv_settings.tunnel[fd].combined != NULL) {
            tc_pfree(srv_settings.cpool, srv_settings.tunnel[fd].combined);
            srv_settings.tunnel[fd].combined = NULL;
        } else {
            tc_log_info(LOG_NOTICE, 0, "crazy , combined null, fd:%d", fd);
        }
#endif
        if (rev != NULL) {
            ...
            if (srv_settings.tunnel[fd].ev != NULL) {
                ...
                tc_log_info(LOG_NOTICE, 0, "crazy, ev null, fd:%d", fd);
            }
        }
    } else {
...
bool  tc_intercept_check_tunnel_for_single(int fd)
{
    ...
    if (srv_settings.s_fd_num < MAX_SINGLE_CONN_NUM) {
